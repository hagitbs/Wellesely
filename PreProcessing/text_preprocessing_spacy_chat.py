...
filename = current_path+'/PreProcessing/WordFrequencies_data_chat.csv' 
print (filename) 
...
print(dflocal)
dflocal.to_csv('agg_local_chat.csv', index=True)
dfglobal.to_csv('agg_global_chat.csv', index=True)

 
# Merge dflocal and dfglobal to the same CSV file  
# Join 2 csv's
df1 = pd.read_csv('agg_local_chat.csv')
df2 = pd.read_csv('agg_global_chat.csv')
print (df1)
merged_df = pd.merge(df1, df2, on='word',   how='outer')
print(merged_df) 
merged_df.to_csv('agg_merged_chat.csv', index=False)